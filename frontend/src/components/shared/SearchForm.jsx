...
const inputBoxStyle = { background: '#f8f9fb', border: '1px solid #e5e7eb', borderRadius: 8, padding: '20px 22px', fontSize: 18, width: '100%', height: 64 };
  ...
  <div style={{ background: '#fff', borderRadius: 16, boxShadow: '0 4px 24px rgba(0,0,0,0.07)', padding: 32, maxWidth: 900, margin: '0 auto 2rem auto' }}>
    <form onSubmit={e => { e.preventDefault(); onSearch(); }} style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <input
        ...
        style={{ ...inputBoxStyle, marginBottom: 12, height: 48, padding: '16px 18px' }}
      />
      ...
      <div style={{ display: 'flex', gap: 12, marginTop: 18 }}>
        <button type="submit" style={{ background: '#4F6AF6', color: '#fff', border: 'none', borderRadius: 8, padding: '10px 24px', fontWeight: 600, cursor: 'pointer', fontSize: 16, display: 'flex', alignItems: 'center', gap: 8 }} disabled={loading}>
          {loading ? 'Searching...' : 'Search'}
        </button>
        <button type="button" style={{ background: '#f5f6fa', color: '#4F6AF6', border: '1px solid #4F6AF6', borderRadius: 8, padding: '10px 24px', fontWeight: 600, cursor: 'pointer', fontSize: 16 }} onClick={onOpenAdvancedFilters}>
          Advanced Filters
        </button>
      </div>
    </form>
