...
const GraphViewLight = ({ darkMode = true }) => {
  ...
  useEffect(() => {
    const handleGlobalKeyDown = (e) => {
      ...
      if (e.key === 'Enter' && !showAuthorModal && !showInstitutionModal) {
        // Trigger if at least one institution is selected AND either an author is selected OR a keyword is entered
        if (selectedInstitutions.length > 0 && (selectedAuthors.length > 0 || searchTerm.trim())) {
  ...
  }, [selectedInstitutions, selectedAuthors, searchTerm, showAuthorModal, showInstitutionModal]);
  ...
  return (
    <div style={{ position: 'relative', minHeight: '100vh', width: '100vw', overflow: 'hidden', background: '#000' }}>
      ...
      <div style={{ position: 'relative', zIndex: 1 }}>
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8, width: '100%' }}>
              ...
              <button
                ...
                disabled={selectedInstitutions.length === 0 || (selectedAuthors.length === 0 && !searchTerm.trim())}
                style={{
                  padding: '10px 24px',
                  fontSize: 16,
                  borderRadius: 6,
                  background: (selectedInstitutions.length === 0 || (selectedAuthors.length === 0 && !searchTerm.trim())) ? '#ccc' : '#4F6AF6',
                  color: '#fff',
                  border: 'none',
                  cursor: (selectedInstitutions.length === 0 || (selectedAuthors.length === 0 && !searchTerm.trim())) ? 'not-allowed' : 'pointer',
        ...
        {hasSearched && (
          <>
            {/* API Call Info Box - above the graph */}
            <div style={{
              background: '#23272f',
              borderRadius: 12,
